...
const BlogSection = () => {
  const mainArticle = {
    ...
    description:
      "Descubrí cómo la Viena de La20 realza los sabores de este clásico alemán. Una combinación que tenés que probar.",
    image: null,
  };
  ...
  return (
    <section className="w-full">
        ...
        <div className="max-w-7xl mx-auto">
          <div className="grid lg:grid-cols-2 gap-8">
            ...
            <article className="bg-white/95 rounded-2xl overflow-hidden shadow-xl hover:shadow-2xl transition-all duration-300 hover:scale-[1.02]">
              <div className="aspect-video overflow-hidden bg-gray-200">{/* Espacio para imagen principal */}</div>
              <div className="p-8">
                <h2 className="text-3xl font-bold text-[hsl(var(--blog-brown))] mb-4 font-deacon uppercase">
                  {mainArticle.title}
                </h2>
                <p className="text-gray-700 text-lg leading-relaxed mb-6">{mainArticle.description}</p>
                <Button
                  variant="default"
                  size="lg"
            ...
            <div className="space-y-6">
              {secondaryArticles.map((article, index) => (
                ...
                >
                  <div className="w-1/3 overflow-hidden flex-shrink-0 bg-gray-200">{/* Espacio para imagen */}</div>
              ...
              {[
                ...
                <CarouselItem key={index} className="pl-4 md:basis-1/2 lg:basis-1/3">
                  <div className="bg-gradient-to-br from-[hsl(var(--blog-orange))] to-[hsl(var(--blog-yellow))] rounded-2xl overflow-hidden shadow-xl hover:shadow-2xl transition-all duration-300 hover:scale-[1.02]">
                    <div className="aspect-[4/3] overflow-hidden bg-white/10">{/* Espacio para imagen */}</div>
                    <div className="p-6">
                      <h3 className="text-xl font-bold text-white mb-2 font-deacon uppercase">{item.title}</h3>
                    </div>
                  </div>
                </CarouselItem>
              ))}
      ...
      <div className="w-full overflow-hidden bg-[hsl(var(--blog-brown))] py-20 px-6">
        <div className="max-w-7xl mx-auto">
